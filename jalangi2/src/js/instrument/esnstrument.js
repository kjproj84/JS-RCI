...
//    }
    ...
    function createCallInitAsStatement(node, name, val, isArgumentSync, lhs, isCatchParam, isAssign) {
        ...
        if (isAssign){
	//	console.log("QQ", RP + "1 = " + logInitFunName + "(" + RP + "2, " + RP + "3, " + RP + "4, " + createBitPattern(isArgumentSync, false, isCatchParam) + ")");
            ret = replaceInStatement(
        ...
        transferLoc(ret[0].expression, node);
       // console.log("createCallInitAsStatement",JSON.stringify(ret));
        return ret;
    }
	...
	//console.log("QQ", logFunctionEnterFunName + "(" + RP + "1,arguments.callee, this, arguments)");
        transferLoc(ret[0].expression, node);
        return ret;
    }
    ...
    function mycreateCallAsScriptEnterStatement(node) {
        printIidToLoc(node);
        var ret = replaceInStatement(logScriptEntryFunName + "(" + RP + "1," + RP + "2, " + RP + "3)",
            getIid(),
            createLiteralAst(instCodeFileName), createLiteralAst(origCodeFileName));
    ...
    function syncDefuns(node, scope, isScript) {
        ...
        if (scope) {
                    ...
                    if (HOP(scope.vars, name)) {
                        if (scope.vars[name] === "defun") {
                            if (!Config.INSTR_INIT || Config.INSTR_INIT(node)) {
                                ...
                                ident = createIdentifierAst(name);
                                ident.loc = scope.funLocs[name];
				//console.log("QQ",   createExpressionStatement(ident, wrapLiteral(ident, ident, N_LOG_FUNCTION_LIT)));
                                ret = ret.concat(
                        ...
                        if (scope.vars[name] === "var") {
                            if (!Config.INSTR_INIT || Config.INSTR_INIT(node)) {
			//	console.log("QQ",  
                         //   		JSON.stringify(createCallInitAsStatement(node,
                         ///           createLiteralAst(name),
                          //          createIdentifierAst(name),
			...
			//			);
                                /**
                                if(name) {
                                    var aaa = "var "+name+"JSRCI123=999999;";
                                    var newnode = acorn.parse(aaa).body[0];
                                }
                                 **/
                                ret = ret.concat(createCallInitAsStatement(node,
                                    createLiteralAst(name),
...
//    }
    ...
    function instrumentScriptEntryExit(node, body0) {
        ...
        // var aaa = "var JSRCI123=222222;";
        // var newnode = acorn.parse(aaa).body[0];

        //console.log("QQ ", escodegen.generate(node),"\n------------------------------------------------------------------------------------------------------\n");
        body = body.concat(syncDefuns(node, scope, true)).
            concat(body0);

        //body = body.concat(newnode);

        //body = body.concat(syncDefuns(node, scope, true)).concat("//JS_RCI test;");
        return body;
    }
    ...
    var visitorRRPost = {
        ...
        "VariableDeclaration": function (node) {
            ...
            var declarations = MAP(node.declarations, function (def) {
                if (def.init !== null) {
                   ...
                   // console.log("INIT", createLiteralAst(def.id.name));
	    ...
	    //var ident = createIdentifierAst("JSRCI");
	    //var bbb= createExpressionStatement(ident, "console.log(\"JSRCI\");");
	    //console.log("QQ", escodegen.generate(bbb));

           //node = node.concat(bbb);
            return node;
        },
...
//        var newAst = esprima.parse(code, {loc:true, range:true});
        var newAst = acorn.parse(code, {locations: true, ecmaVersion: 7 });
...
//        console.log(JSON.stringify(newAst,null,"  "));
    ...
    function instrumentCode(options) {
        ...
        if(options.origCodeFileName.includes("node_modules")){
            console.log("@@@@@@@@@@",options.origCodeFileName);
            skip = true;
        }
        ...
        if (!skip && typeof code === 'string' && code.indexOf(noInstr) < 0) {
            try {
                ...
                console.log("Failed to instrument\n", code);
                throw ex;
            }
        }
