...
function iterCFG(nd, f) {
  ...
  case 'ExpressionStatement':
    ...
    case 'CallExpression':
      ...
      if(nd.expression.arguments) {
// rec(nd.expression.right);
        for(var arg in nd.expression.arguments){
        console.log("FunctionExpression11 ", nd.expression.arguments[arg].type);
          if(nd.expression.arguments[arg].type=='FunctionExpression'){
            // f(nd.expression.arguments[arg]);
            // f(ast.getAttribute(nd.expression.arguments[arg], 'fakeRoot'));
            rec(nd.expression.arguments[arg]);
            // break;
          }
...
function dumpCFG(root) {
  ...
  iterCFG(root, function(nd) {
    ...
    if(sets.size(succs) === 0) {
      if(nd && nd.type!="FunctionDeclaration"){
        // res += dumpNode(nd) + " --> []\n";
          }
    } else {
      // if(nd && nd.type!="FunctionDeclaration" && succs && succs.type!="FunctionDeclaration")
      //   var r1 = dumpNode(nd) + " --> [" + sets.map(succs, dumpNode).join(', ') + "]\n";
      // if(r1 && !r1.includes("FunctionDeclaration")) {
          res += dumpNode(nd) + " --> [" + sets.map(succs, dumpNode).join(', ') + "]\n";
          // tt = false;
      // }
    }
    // if(tt && idom && idom.type && idom.type!="FunctionDeclaration") {
      // var r1 = "    immediate dominator: " + (idom ? dumpNode(idom) : "none") + "\n";

        res += "    immediate dominator: " + (idom ? dumpNode(idom) : "none") + "\n";
    // }
    // if(tt&&  ipdom && ipdom.type && ipdom.type!="FunctionDeclaration") {
...
var fs = require('fs');
var filename ="../../copy/mock-properties.js";
var code = fs.readFileSync(filename).toString();
 console.log(code);
  var ast11 = esprima.parse(code, { loc: true, range: true });
  cfg.buildCFG(ast11);
  dominators.buildDominatorTrees(ast11, true);
  var actual = dumpCFG(ast11);
