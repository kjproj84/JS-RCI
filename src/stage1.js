...
temporalNameMaker.prototype = {
    makeVariableName: function () {
                          "use strict";
                          return 'tmpv'+this.variableNameCnt++;
                      },
...
module.exports = {
    phase0: function (code) {
        ...
        **/
        ...
        var phase0B = falafel (code, function(node) {
            var tempUpdate;
            if (node.arguments !== undefined && node.callee &&  node.callee.name!="require") {
                // console.log(node);
        ...
        var phase0D = falafel(phase0C, function(node) {
            var tempSplit = null;
                  //console.log(checkCalleeProperty(node));
        ...
        var phase0E = falafel(phase0D, function(node) {
            var tempSplit;
             // if(checkCalleeProperty(node)==false)
                            var aaa = (node.start == 59 && node.end == 99);
                            // console.log("@@", node.type, node.start, node.end, node.source(),checkCalleeProperty(node));
// if (aaa) {
            if (node.type === 'CallExpression' || node.type === 'MemberExpression') {
                    ...
                    // tempSplit = null;
                    if (checkCalleeProperty(node) && checkMinConCallOrMem(node)) {
                        tempSplit = SplitExpression(node);
                        // console.log("       tempSplit   ",tempSplit, node.source());
                        // if(aaa)
                        //     tempSplit = null;
                        if (tempSplit !== null) {
...
function checkCalleeProperty(expression) {
        // if("knex.raw(tmpv3).then"==expression.source())
        // console.log("##",expression.source());
        if(expression.property ){
          //  console.log(expression.callee.source());
            if(expression.property.name ==="then") {
                // console.log("       ",true, true, expression.source());
                return false;}
       //     else if(expression.callee.object.type ==="CallExpression" || expression.callee.object.type ==="MemberExpression"){
            else if(expression.callee){

                checkCalleeProperty(expression.callee);
            }
        }
        else if(expression.callee && expression.callee.property){
            if(expression.callee.property.name =="then")
               return false;
            checkCalleeProperty(expression.callee);
        }
        return true;
